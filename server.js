...
app.use(session({
  secret: "roland-dg-brasil-flow-secret",
  resave: false,
...
app.get("/api/meta", auth, (req, res) => {
  res.json({
    ...
    statuses: [
      ...
      "Em análise importação",
      "Aprovado parcialmente",
      "Aprovado pela importação",
      "Reprovado pela importação",
      "Concluído"
    ],
    urgencyLevels: ["Baixa", "Média", "Alta", "Crítica"]
  });
});

app.get("/api/requests", auth, (req, res) => {
  const items = db.getRequests().filter(r => requestForRole(req.user, r));
  res.json(items);
});
...
app.post("/api/requests", auth, allow("requester"), (req, res) => {
  ...
  db.appendHistory(newRequest.id, {
    by: req.user.name,
    role: roleLabels[req.user.role],
    action: "Solicitação criada",
    detail: `Pedido criado com ${newRequest.quantityRequested} unidade(s).`
  });
...
app.put("/api/requests/:id/approve", auth, allow("approver"), (req, res) => {
  ...
  db.appendHistory(id, {
    ...
    detail: req.body.comment || "Sem observações."
  });
  res.json(updated);
});
...
app.put("/api/requests/:id/return", auth, allow("approver"), (req, res) => {
  ...
  db.appendHistory(id, {
    by: req.user.name,
    role: roleLabels[req.user.role],
    action: "Devolvido para ajuste",
    detail: req.body.comment || "Revisar informações."
  });
  res.json(updated);
});
...
app.put("/api/requests/:id/reject", auth, allow("approver"), (req, res) => {
  ...
  db.appendHistory(id, {
    by: req.user.name,
    role: roleLabels[req.user.role],
    action: "Reprovado pelo gestor",
    detail: req.body.comment || "Solicitação reprovada."
  });
  res.json(updated);
});
...
app.put("/api/requests/:id/import", auth, allow("import"), (req, res) => {
  ...
  if (!allowed.includes(importStatus)) {
    return res.status(400).json({ error: "Status inválido para importação." });
  }
...
app.listen(PORT, () => {
  console.log(`Roland DG Brasil Flow rodando em http://localhost:${PORT}`);
});